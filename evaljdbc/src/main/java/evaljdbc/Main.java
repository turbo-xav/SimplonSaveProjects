...
public class Main {
		...
		while(catId == 0) {
			...
			else
			{
				...
				if(category.getFoods().size() > 0) {
					System.err.println("Vous ne pouvez pas supprimer la catégorie : "+category.getName()+ " car elle contient encore des aliments !!");
					displayCats();
					return;
				}
	...
	public static void deleteFood() {
		...
		if(nbFoods == 0) {
			System.err.println("Opération impossible car aucun aliment n'est présent dans la base de données");
			return;
		}
